...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function displayRecipe(recipe) {
        ...
        recipeTime.textContent = `${recipe.cookingTime} mins`;
        ...
        if (recipe.cuisine && recipe.cuisine !== 'other') {
            const cuisineTag = document.createElement('span');
            cuisineTag.className = 'recipe-tag';
            cuisineTag.textContent = recipe.cuisine;
            recipeTags.appendChild(cuisineTag);
        }
    ...
    function fetchAuthor(userId) {
        fetch(`/api/users/${userId}`)
            ...
            .then(user => {
                recipeAuthor.textContent = user.name || 'Unknown';
            })
            .catch(error => {
